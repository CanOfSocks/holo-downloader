...
END
    ...
    if [ -n "${VIDEOSCHEDULE}" ]; then
        CRON_CONTENT+="${VIDEOSCHEDULE} /app/getVids.py 'spawn' > /proc/1/fd/1 2>/proc/1/fd/2"$'\n'
    else
        CRON_CONTENT+="*/2 * * * * /app/getVids.py 'spawn' > /proc/1/fd/1 2>/proc/1/fd/2"$'\n'
    fi

    if [ -n "${MEMBERSCHEDULE}" ]; then
        CRON_CONTENT+="${MEMBERSCHEDULE} /app/getMembers.py 'spawn' > /proc/1/fd/1 2>/proc/1/fd/2"$'\n'
    else
    ...
    if [ -n "${UNARCHIVEDSCHEDULE}" ]; then
        CRON_CONTENT+="${UNARCHIVEDSCHEDULE} /app/communityPosts.py > /proc/1/fd/1 2>/proc/1/fd/2"$'\n'
    else
        CRON_CONTENT+="*/30 * * * * /app/communityPosts.py > /proc/1/fd/1 2>/proc/1/fd/2"$'\n'
    fi
