...
partialoutput=$(yt-dlp --cookies /app/cookies.txt --wait-for-video 1-300 \
--skip-download -o "%(channel)s/[%(upload_date)s] %(title)s - %(channel)s (%(id)s)/[%(upload_date)s] %(title)s - %(channel)s (%(id)s)" --print "%(filename)s" \
--no-warnings "$@" 2>&1 | tail -n 1)
