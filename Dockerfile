...
RUN apt-get update && apt-get install --no-install-recommends wget unzip xz-utils procps cron git gcc -y -qq && apt clean -y
...
RUN apt-get purge git gcc -y && apt-get autopurge -y
